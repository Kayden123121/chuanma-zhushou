...
def format_opportunities(opportunities):
    ...
    if watch_list:
        ...
        for item in watch_list:
            ...
            lines.append(f"    * {action} {tile} -> 切 {discard} (听牌距离: {item.get('shanten_after')})")
    ...
    if keep_list:
        ...
        for item in keep_list[:5]:
            lines.append(f"    * 摸 {item['draw']} -> 切 {item['discard']} (听牌距离: {item['shanten']}, 进张: {item['ukeire']})")
...
def run_complex_analysis(engine):
    # 川麻：14 张万筒条
    print("\n[测试用例分析·川麻]")
    hand_line = "3467m2356p5578s12m"
    print(f"手牌: {hand_line}")
    ids = TilesConverter.one_line_string_to_136_array(hand_line)
    ...
    if result:
        ...
        print(f"听牌距离 (切牌后): {result['shanten']}")
        print(f"进张: {result['ukeire']}")
